...
class GFCoupons {
    ...
    public static function enqueue_coupon_script($form, $is_ajax){
        ...
        wp_enqueue_script("coupon_script", self::get_base_url() . "/js/coupons.js", array("jquery"), self::$version);
    ...
    public static function coupon_input($input, $field, $value, $lead_id, $form_id){
        ...
        if(RG_CURRENT_VIEW == "entry")
            return __("Coupon fields are not editable", "gravityformscoupons");
        ...
        $input = "<div class='ginput_container' id='gf_coupons_container_{$form_id}'>" .
                    ...
                    "<input type='button' disabled='true' onclick='ApplyCouponCode(" . $form_id . ");' value='" . __("Apply", "gravityformscoupon") . "' id='gf_coupon_button' class='button' " . disabled(is_admin(), true, false) . " " . GFCommon::get_tabindex() . "/> ".
                        ...
                        if(is_array($feeds) && sizeof($feeds) > 0){
                            foreach($feeds as $feed){
                                ...
                                <tr class='author-self status-inherit' valign="top">
                                    ...
                                    <td class="column-title"><?php echo empty($feed["form_title"]) ? __("Any Form", "gravityformscoupon") : $feed["form_title"]; ?></td>
            ...
            <form method="post" action="">
                ...
                <div id="coupons_form_container" valign="top" >
                    <table class="gforms_form_settings" style="width:600px;">
                    	<tr>
							<td colspan="2"><h4 class="gf_settings_subgroup_title"><?php _e("Applies to Which Form?", "gravityformscoupon")?></h4></td>
						</tr>
                        <tr>
                            <th>
                ...
                <div id="coupons_field_group" valign="top" <?php echo rgblank(rgar($config, "form_id")) ? "style='display:none;'" : "" ?>>
                    ...
                    <table class="gforms_form_settings">
                    	<tr>
							<td colspan="2"><h4 class="gf_settings_subgroup_title"><?php _e("Coupon Basics", "gravityformscoupon")?></h4></td>
						</tr>
                        ...
                        <tr <?php echo ($is_validation_error && empty($config["meta"]["coupon_name"]) ? "class=\"coupon_validation_error\"" : "" )?> >
                            <th >
                                <label for="gf_coupon_name">
                                    ...
                                    <span class="description">(<?php _e("required", "gravityformscoupon")?>)</span>
                        ...
                        <tr>
							<td colspan="2"><h4 class="gf_settings_subgroup_title"><?php _e("Coupon Options", "gravityformscoupon")?></h4></td>
						</tr>
    ...
    public static function add_discounts($product_info, $form, $lead){
        ...
        foreach($coupons as $coupon){
            ...
            $product_info["products"][$coupon["code"]] = array(
                                                            ...
                                                            "quantity" => 1,
                                                            "options" => array(
                                                                            array(  "option_label" => __("Coupon Code:", "gravityformscoupon") . " " . $coupon["code"],
                                                                                    "price" => 0)
                                                                            )
                                                            );
        }
    ...
    public static function can_apply_coupon($coupon_code, $existing_coupon_codes, $config, &$invalid_reason="", $form){
        ...
        foreach($existing_coupons as $existing_coupon){
            if(!$existing_coupon["can_stack"] || !$coupon["can_stack"]){
                $invalid_reason = __("This coupon can't be used in conjuction with other coupons you have already entered.", "gravitycharge");
                return false;
            }
        }
    ...
    public static function is_valid_code($code, $config, &$invalid_reason=""){
        ...
        if(!$code_exists){
            $invalid_reason = __("Invalid coupon.", "gravitycharge");
            return false;
        }
    ...
    public static function is_valid($config, &$invalid_reason=""){
        ...
        $start_date = mysql2date("G", $config["meta"]["coupon_start"]);
        $end_date = mysql2date("G", $config["meta"]["coupon_expiration"]);
        $now = time();
        ...
        if(empty($config["meta"]["coupon_limit"]) || $coupon_usage < intval($config["meta"]["coupon_limit"]))
            $is_under_limit = true;
        if(!$is_under_limit){
            $invalid_reason = __("This coupon has reached its usage limit.", "gravitycharge");
            return false;
        }
